...
@pytest.mark.parametrize("parens, output_bool", [
         ("()", True),
    ("()[]{}", True),
    ("(]", False),
    ("){", False)
...
@pytest.mark.parametrize("funcs, params", [
...
@pytest.mark.parametrize("tokens,output", [
    (["2", "1", "+", "3", "*"], 9),
    (["4", "13", "5", "/", "+"], 6),
    (["10", "6", "9", "3", "+", "-11", "*", "/", "*", "17", "+", "5", "+"], 22),
])
...
@pytest.mark.parametrize("temperatures, output", [
...
@pytest.mark.parametrize("target, position, speed, output", [
